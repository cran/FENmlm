...
vignette: >
  %\VignetteIndexEntry{Fixed-Effects ML Estimation}
...
The package `FENmlm` is devoted to the estimation of maximum likelihood (ML) models with fixed-effects. 
The function `femlm` is the workhorse of the package: it performs efficient ML estimations with *any number* of fixed-effects and also allows for non-linear in parameters right-hand-sides. Four likelihood models are supported: Poisson, Negative Binomial, Gaussian and Logit. 
...
The main feautres of the package are illustrated in this vignette.

# Complete example using Trade data
...
This data set focuses on the levels of bilateral importations between EU15 countries from 2007 and 2016. The data is further broken down according to 20 product categories.
...
Note that when you use the Poisson/Negative Binomial families, this relationship is in fact linear because the right hand side is exponentialized to avoid negative values for the Poisson parameter. This leads to the equivalent relation:
...
Note that you can always cluster the standard-errors, even when not estimating with fixed-effects. Buth then you need to use the argument `cluster`, as before:
...
The previous code performs 5 estimations with an increasing number of fixed-effects and store their results into the list named `gravity_subcluster`. To show the results of all 5 estimations, it's easy:
...
## Looking at the fixed-effects

To obtain the fixed-effects of the estimation, the function `getFE` must be performed on the results. The `print` method helps to have a quick overview:
...
To estimate the non-linear relationship, we need to use the argument `NL.fml` where we put the non-linear part. We also have to provide starting values with the argument `start`. Finally, to ensure the RHS can be evaluated in any situation, we add lower bounds for the parameters with the argument `lower`.

```{r}
result_NL = femlm(z~0, base, NL.fml = ~ log(a*x + b*y), start = list(a=1, b=1), lower = list(a=0, b=0))
```

Note that the arguments `start` and `lower` are named lists. Setting `lower = list(a=0, b=0)` means that the optimization algorithm will never explores parameters for $a$ and $b$ that are lower than 0.
...
Further, because $\gamma_{id_i} \left( a\times x_i + b\times y_i \right)$ is equivalent to $a\times \gamma_{id_i} \left(x_i + b/a\times y_i \right)$, estimating the first relation directly will lead to a problem to uniquely identify the estimates (i.e. there is an infinity of $\gamma_{id_i}$, $a$ and $b$ that will solve it). We will fix one of the coefficient to uniquely identify them. 
...
The user ought to estimate the coefficient of variables that are not collinear: 1) among each other, nor 2) with the fixed-effects. In such cases, the algorithm raise a warning and point to the collinear variable.

Let's take an example in which we want to estimate the coefficient of a variable which is constant. Of course it makes no sense, so a warning will be raised and the suspicious variable will be named.
...
In such situation, just remove the collinear variables. Their identification is eased by the detailed warning. Even in more elaborate cases of collinearity, the algorithm tries to find out the culprit and inform the user accordingly. 
...
However, in very specific cases: for large data sets and very "similar" fixed-effects (ie fixed-effects with many overlap), the algorithm may not converge because because the fixed-effects are not computed with enough precision (although they'll be very close to optimality). 

In such cases, just set the stopping criterion with the argument `precision.cluster` to a lower value (the algorithm will take more time to run though because there will be more iterations).
