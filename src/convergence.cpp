...
void CCC_negbin(int nb_cluster, double theta, double diffMax_NR,
                double *cluster_coef, double *mu,
	...
	int iterMax = 100, iter = 0, iterFullDicho = 10;
	double exp_mu;
	...
	for(m=0 ; m<nb_cluster ; m++){
		...
		borne_inf[m] = log(sum_y[m]) - log(double(table[m])) - mu_max;
	...
	for(m=0 ; m<nb_cluster ; m++){
		...
		// the bounds
		lower_bound = borne_inf[m];
		upper_bound = borne_sup[m];
...
void CCC_logit(int nb_cluster, double diffMax_NR,
               double *cluster_coef, double *mu,
...
void computeClusterCoef_single(int family, int n_obs, int nb_cluster, double theta, double diffMax_NR,
                               ...
                               int *dum, int *obsCluster, int *table, int *cumtable, int nbThreads){
	...
	case 2: // Negbin
		if(nbThreads == 1){
			CCC_negbin(nb_cluster, theta, diffMax_NR, cluster_coef, mu, lhs, sum_y, obsCluster, table, cumtable);
		} else {
			CCC_par_negbin(nbThreads, nb_cluster, theta, diffMax_NR, cluster_coef, mu, lhs, sum_y, obsCluster, table, cumtable);
		}
		break;
	case 3: // logit
		if(nbThreads == 1){
			CCC_logit(nb_cluster, diffMax_NR, cluster_coef, mu, sum_y, obsCluster, table, cumtable);
		} else {
			CCC_par_logit(nbThreads, nb_cluster, diffMax_NR, cluster_coef, mu, sum_y, obsCluster, table, cumtable);
		}
		break;
	case 4: // Gaussian
...
SEXP compute_cluster_coef_r(int family, int nb_coef, double theta, double diffMax_NR,
                            ...
                            SEXP r_obsCluster, SEXP r_table, SEXP r_cumtable, int nbThreads){
	...
	computeClusterCoef_single(family, n_obs, nb_coef, theta, diffMax_NR,
                           pcoef, mu, lhs, sum_y, dum, obsCluster, table, cumtable, nbThreads);
...
SEXP update_mu_single_cluster(int family, int nb_cluster, double theta, double diffMax_NR, SEXP mu_in,
	...
	computeClusterCoef_single(family, n_obs, nb_cluster, theta, diffMax_NR, cluster_coef.data(), pmu_in,
                           plhs, psum_y, pdum, pobsCluster, ptable, pcumtable, nbThreads);
...
void computeClusterCoef(vector<double*> &pcluster_origin, vector<double*> &pcluster_destination,
	...
	for(k=K-1 ; k>=0 ; k--){
		...
		computeClusterCoef_single(family, n_obs, nb_cluster, theta, diffMax_NR,
                            ...
                            my_dum, my_obsCluster, my_table, my_cumtable, nbThreads);
...
List cpp_conv_acc_gau_2(int n_i, int n_j, int n_cells,
	...
	//
	// const_a and const_b
	vector<double> const_a(n_i);
	vector<double> const_b(n_j);
	int *dum_i = INTEGER(dum_vector);
	int *dum_j = dum_i + n_obs;
	double *invTable_i = REAL(invTableCluster_vector);
	double *invTable_j = invTable_i + n_i;
